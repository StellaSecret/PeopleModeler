...
class DriveSync(
    ...
    private val backupFilename = "people_modeler_backup.json"
    private val mimeJson = "application/json"
    private val folderAppdata = "appDataFolder"
    ...
    private fun buildDriveService(): Drive? {
        val credential =
            authManager.getDriveCredential() ?: run {
    ...
    suspend fun backup(persons: List<Person>): SyncResult =
        withContext(Dispatchers.IO) {
            ...
            return@withContext try {
                ...
                val content = ByteArrayContent(mimeJson, json.toByteArray(Charsets.UTF_8))
    ...
    suspend fun restore(): Pair<SyncResult, List<Person>?> =
        withContext(Dispatchers.IO) {
    ...
    suspend fun getBackupInfo(): BackupInfo? =
        withContext(Dispatchers.IO) {
