...
mod tests {
    ...
    #[test]
    fn test_bias_label_all_variants_exact_values() {
        use crate::wasm;
        let cases: &[(&str, &str, &str)] = &[
            ("CONFIRMATION", "Biais de confirmation", "Confirmation bias"),
            ("ANCHORING", "Ancrage cognitif", "Anchoring"),
            ("AVAILABILITY", "Disponibilité", "Availability"),
            ("SUNK_COST", "Coût irrécupérable", "Sunk cost"),
            ("DUNNING_KRUGER", "Dunning-Kruger", "Dunning-Kruger"),
            ("IMPOSTOR", "Imposteur", "Impostor"),
            ("LOSS_AVERSION", "Aversion aux pertes", "Loss aversion"),
            ("SOCIAL_PROOF", "Preuve sociale", "Social proof"),
            ("AUTHORITY", "Autorité", "Authority"),
            ("RECENCY", "Récence", "Recency"),
            ("IN_GROUP", "Endogroupe", "In-group"),
        ];
        for &(id, fr, en) in cases {
            assert_eq!(wasm::bias_label(id, "fr"), fr, "FR label for {}", id);
            assert_eq!(wasm::bias_label(id, "en"), en, "EN label for {}", id);
        }
    }
