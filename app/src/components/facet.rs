...
/// Two-state Base/Work facet radiogroup shared by the edit form, the people
/// list and the person detail header. Previously each page hand-copied the
/// same `.facet-toggle` markup (a third copy in team.rs adds an "Automatic"
/// state over `Option<FacetKind>`, so it deliberately keeps its own).
#[component]
pub fn FacetToggle(
