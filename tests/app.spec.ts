...
test.describe('Web App', () => {
  ...
  test('affiche le bouton Créer un profil', async ({ page }) => {
    await expect(page.locator('button:has-text("Créer un profil")')).toBeVisible();
  });
  ...
  test('l\'interprétation OCEAN se met à jour', async ({ page }) => {
    ...
    await slider.fill('9');
    await slider.dispatchEvent('input');
    await expect(page.locator('#oceanInterp')).toContainText('extraverti');
  });
  ...
  test('insights — sélectionner un contexte affiche une analyse', async ({ page }) => {
    ...
    await expect(page.locator('#insightOutput')).not.toContainText('Sélectionnez un contexte');
  });
  ...
  test('le bouton Export déclenche un téléchargement', async ({ page }) => {
    ...
    const [download] = await Promise.all([
      ...
      page.click('button:has-text("Exporter JSON")'),
    ]);
